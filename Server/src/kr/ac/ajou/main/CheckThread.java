...
public class CheckThread extends Thread {
    ...
    while (true) {
      ...
      if (diceNum1 != diceNum2) {
        ...
        try {
          ...
          if (clientOneTurn) {
            sessionThreadList.get(FIRST_CLIENT).myTurnQueue.add(clientOneTurn);
          }
          ...
          if (clientTwoTurn) {
            sessionThreadList.get(SECOND_CLIENT).myTurnQueue.add(clientTwoTurn);
          }
  ...
  private void setGameStateGameOver() {
    gameState = new GameState(GameState.GAME_OVER);
    System.out.println("gameState: GAME_OVER");
  }

  static synchronized int clientConnected() {
    return ++clientCount;
  }
